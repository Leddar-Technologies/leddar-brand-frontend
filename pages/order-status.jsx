...
import { formatNaira, formatVatPercent, calculateTotalWithVat } from "../utils/pricing";
...
export default function OrderStatusPage() {
  ...
  async function handleOpenPayment(quote) {
  ...
  async function handleInitiatePayment(e) {
    ...
    try {
      // For production quotes the orderId comes from the linked order
      const orderId = selectedQuote.orders?.[0]?.id;
      if (!orderId) throw new Error("No order linked to this quote yet. Please contact support.");

      const result = await initializeProductionPayment({ email: payEmail, orderId });
  ...
  const productionQuotes = quotes.filter((q) => q.type === "PRODUCTION");
      ...
      {loading ? (
        ...
        <div className="mt-6 space-y-4">
          {productionQuotes.map((quote) => {
            const badge   = STATUS_BADGE[quote.status] || STATUS_BADGE.SUBMITTED;
            const hasPrice = !!quote.price;
            const vat      = hasPrice ? calculateTotalWithVat(quote.price).vatAmount : 0;
            const total    = hasPrice ? calculateTotalWithVat(quote.price).totalAmount : 0;
            const isPaid   = quote.orders?.[0]?.status === "IN_PRODUCTION" || quote.orders?.[0]?.status === "DELIVERED";
              ...
              <div key={quote.id} className="card p-5">
                ...
                {hasPrice ? (
                  <div className="mt-4 grid gap-2 rounded-xl border border-[#E6D7CB] bg-[#FFF8EF] p-4 text-sm sm:grid-cols-2">
                    ...
                    <div>
                      <p className="text-xs uppercase tracking-[0.12em] text-[#8B6A39]">VAT ({formatVatPercent()})</p>
                      <p className="mt-0.5 font-semibold text-ink">{formatNaira(vat)}</p>
                    </div>
                    <div className="border-t border-[#E6D7CB] pt-2 sm:col-span-2">
                      <p className="text-xs uppercase tracking-[0.12em] text-[#8B6A39]">Total Payable</p>
